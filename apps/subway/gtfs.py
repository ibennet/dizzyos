"""A minimal GTFS-realtime reader — just the fields the subway app needs.

The MTA publishes realtime arrivals as protobuf. Rather than take on a protobuf
dependency and generated bindings for a schema we use three fields of, this walks
the wire format directly and picks out those fields by number. Field numbers are
...
def iter_arrivals(buf):
    ...
    feed. `stop_id` keeps its direction suffix (e.g. "F11N"); arrival time is used
    when present, departure as the fallback."""
    out = []
